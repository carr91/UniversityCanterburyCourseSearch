...
def parse_course_details(soup):
    """Parse restrictions, equivalent courses, and contact person from the course details page."""
    restrictions = []
    equivalents = []
    contact_person = []
    ...
    if contact_person_section:
        ...
        if contact_person_link:
            contact_person.append(contact_person_link.text)

    return restrictions, equivalents, contact_person

def process_course_info(input_file, output_file):
    """Process course_info.csv to add Restrictions, Equivalents, and Contact Person."""
    with open(input_file, "r", encoding="utf-8") as infile, open(output_file, "w", encoding="utf-8", newline="") as outfile:
        ...
        header = next(reader)
        header.extend(["Restrictions", "Equivalents", "Contact Person"])
        ...
        for row in reader:
            ...
            for course_code in course_codes:
                ...
                if soup:
                    course_restrictions, course_equivalents, course_contact_person = parse_course_details(soup)
...
if __name__ == "__main__":
    ...
    # Write the header without the "Description" field
    ...
    # Combine courses with the same title
    combined_courses = combine_courses_by_title_and_info(unique_courses)
    
    # Fixed sorting logic
    sorted_courses = sort_courses_by_code(combined_courses)
    
    # Write the data without the "Description" field
    with open(output_file, "a", encoding="utf-8", newline='') as f:
        writer = csv.writer(f)
        for course in sorted_courses:
            writer.writerow([course[0], course[1], course[3], course[4]])  # Exclude course[2] (Description)
    
    print(f"Results From search  saved to {output_file} , Parsing each course for leacturers and reqs now...")
    input_file = "course_info.csv"
    output_file = "course_info_with_details.csv"
    process_course_info(input_file, output_file)
    print(f"Updated course info saved to {output_file}")
