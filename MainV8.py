...
def main(keywords):
    ...
    # Write the initial data without the "Description" field
    with open(output_file, "a", encoding="utf-8", newline='') as f:
        writer = csv.writer(f)
        for course in sorted_courses:
            writer.writerow([course[0], course[1], course[3], course[4]])  # Exclude course[2] (Description)
    
    print(f"Results from search saved to {output_file}, Parsing each course for lecturers and requirements now...")
    ...
    print(f"Final updated course info saved to {output_file}")
